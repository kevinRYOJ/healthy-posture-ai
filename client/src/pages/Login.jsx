  ...
  return (
    <main className="min-h-[calc(100vh-70px)] flex items-center justify-center p-8 px-6 bg-[radial-gradient(ellipse_at_60%_0%,#CCFBF1_0%,var(--color-bg)_60%)]">
      <div className="bg-white rounded-lg shadow-md border border-border w-full max-w-[440px] flex flex-col gap-4 p-12 animate-fade-in">
        ...
        <h1 className="text-[1.6rem] font-extrabold text-text -mt-2 font-heading">Selamat datang kembali</h1>
        <p className="text-[0.88rem] text-text-secondary -mt-2">Masuk untuk melanjutkan tracking posturmu</p>
        ...
        <form className="flex flex-col gap-4 mt-2" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-[0.35rem]">
            <label htmlFor="password" className="text-[0.83rem] font-semibold text-text flex items-center justify-between">Password</label>
            <input
              id="password"
              name="password"
              type="password"
        ...
        <p className="text-center text-[0.875rem] text-text-secondary pt-2 border-t border-border">
          Belum punya akun?{' '}
          <Link to="/register" className="text-primary font-semibold underline underline-offset-2 hover:text-primary-dark">Daftar sekarang</Link>
        </p>
      </div>
    </main>
  );
